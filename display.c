...
/*   Updated: 2022/07/26 16:40:20 by pgeeser          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{	
	...
	while (y++ < WIN_HEIGHT)
	{
		...
		while (x++ < WIN_WIDTH)
		{
			...
			if (frac == vars->iterations || (!vars->mandelbrot && ((y > (WIN_HEIGHT - (35 + 7)) && x > WIN_WIDTH - ((20 * 7.3) + 7)) || (y > (WIN_HEIGHT - (85 + 7)) && x < (30 * 7.3) + 7))) || (vars->mandelbrot && ((y > (WIN_HEIGHT - (35 + 7)) && x > WIN_WIDTH - (20 * 7.3 + 7)) || (y > (WIN_HEIGHT - (65 + 7)) && x < (30 * 7.3) + 7))))
				*((unsigned int *)(vars->img.addr + ((WIN_HEIGHT - y - 1)
								* WIN_WIDTH + x - 1))) = 0;
			else
				*((unsigned int *)(vars->img.addr + ((WIN_HEIGHT - y - 1)
								* WIN_WIDTH + x - 1))) = create_color(25,
						255 / vars->frac_color, 100 / vars->frac_color, 0)
					* frac / vars->frac_color;
		}
	}
}
